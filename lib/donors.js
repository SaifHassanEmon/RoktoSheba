...
export function getDivisionForDistrict(districtName) {
  ...
  for (const [divName, divData] of Object.entries(BANGLADESH_DATA)) {
    if (divData.districts && divData.districts[districtName]) {
      return divName;
    }
  }
  return 'Dhaka';
...
export async function getDonors(filters = {}) {
  ...
  try {
    ...
    // Post-query filter for division (handles legacy database documents)
    if (filters.division && filters.division !== 'all') {
      results = results.filter(d => {
        const donorDivision = d.division || getDivisionForDistrict(d.district);
        return donorDivision === filters.division;
      });
    }

    // Post-query filter for district (handles legacy database documents)
    if (filters.district && filters.district !== 'all') {
      results = results.filter(d => {
        const donorDistrict = d.district || 'Dhaka';
        return donorDistrict === filters.district;
      });
    }

    // Post-query filter for area (supports both legacy string and new array-of-areas)
    if (filters.area && filters.area !== 'all') {
      results = results.filter(d => {
        const matchPrimary = d.area === filters.area;
        const matchAvailable = Array.isArray(d.areas) && d.areas.includes(filters.area);
        return matchPrimary || matchAvailable;
      });
    }
    ...
    // Post-query filter for availability (ensures ineligible donors are not considered available)
    if (filters.availableOnly) {
      results = results.filter(d => d.available && isDonorEligible(d.lastDonation));
    }
...
export async function getCompatibleDonors(bloodGroup) {
  ...
  try {
    ...
    // Post-filter to ensure only eligible donors are returned
    return results.filter(d => isDonorEligible(d.lastDonation));
  } catch (error) {
