...
async function sendAreaButtons(senderPsid, bloodGroup) {
  ...
  userSessions.set(senderPsid, { step: 'AWAITING_AREA', bloodGroup });
...
// Sessions are handled globally at the top of the file
...
export async function handleMessage(senderPsid, receivedMessage) {
  ...
  if (messageText) {
    ...
    // 2. If no keywords match, check if there is an active session awaiting area input
    const session = userSessions.get(senderPsid);
    if (session && session.step === 'AWAITING_AREA') {
      // Check if they typed "all" or "all areas"
      if (['all', 'all areas', 'anywhere', 'any'].some(val => messageText.includes(val))) {
        userSessions.delete(senderPsid);
        return sendDonorResults(senderPsid, session.bloodGroup, 'ALL');
      }

      // Find matching area in DHAKA_AREAS case-insensitively
      const matchedArea = DHAKA_AREAS.find(
        a => a.toLowerCase() === messageText
      );

      if (matchedArea) {
        userSessions.delete(senderPsid);
        return sendDonorResults(senderPsid, session.bloodGroup, matchedArea);
      } else {
        // Notify them and let them choose/type again (keep session alive)
        await sendTextMessage(senderPsid, 
          `📍 Sorry, "${receivedMessage.text}" is not a recognized area in Dhaka.\n\nPlease type a valid area (e.g., Dhanmondi, Mirpur, Uttara) or select one of the options below.`
        );
        return sendAreaButtons(senderPsid, session.bloodGroup);
      }
    }
