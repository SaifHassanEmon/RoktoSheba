...
async function sendDonorResults(senderPsid, bloodGroup, area) {
  ...
  const donors = getDonors(filters);
...
export async function handleMessage(senderPsid, receivedMessage) {
  ...
  if (messageText) {
    ...
    // Blood group search (e.g., "need B+ blood", "B+ donor", "I need O-")
    const bgMatch = messageText.match(/\b(a|b|o|ab)[+\-]\b/i);
    if (bgMatch && (messageText.includes('need') || messageText.includes('find') || messageText.includes('donor') || messageText.includes('blood'))) {
      const bloodGroup = bgMatch[0].toUpperCase().replace(/([+-])/, (m) => m);
